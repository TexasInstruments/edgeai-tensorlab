    ...
    def forward(self, inputs):
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            if 'scale_factor' in self.upsample_cfg:
                '''
                if len(laterals[i].shape) == 4:
                    N, C, H, W = laterals[i].shape
                    laterals[i] = laterals[i].view(1, N*C, H, W)
                    upsampled = F.interpolate(laterals[i],
                                              **self.upsample_cfg)
                    #N, C, H, W = upsampled.shape
                    sy, sx = self.upsample_cfg['scale_factor']
                    upsampled = upsampled.view(N, C, sy*H, sx*W)
