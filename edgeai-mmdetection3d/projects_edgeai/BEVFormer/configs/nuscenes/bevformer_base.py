...
#img_norm_cfg = dict(
#    mean=[123.675, 116.28, ], std=[58.395, 57.12, 57.375], bgr_to_rgb=True)
img_norm_cfg = dict(
    mean=[103.53, 116.28, 123.675], std=[1.0, 1.0, 1.0], bgr_to_rgb=False)
...
model = dict(
    ...
    img_backbone=dict(
        ...
        style='caffe',
        dcn=dict(type='DCNv2_tidl', grid_sample_mode='bilinear', deform_groups=1, fallback_on_stride=False), # original DCNv2 will print log when perform load_state_dict
        stage_with_dcn=(False, False, True, True)), # what is different of 'torch' and 'caffe'
    img_neck=dict(
        type='mmdet.FPN',
