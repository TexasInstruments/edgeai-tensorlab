...
class NMSFreeCoder(BaseBBoxCoder):
    ...
    def decode_single(self, cls_scores, bbox_preds):
        ...
        # Not fully verified for PETR
        # Not called for Far3D, StreamPETR (self.bbox_norm_type == 2) due to size mismatch - REVISIT
        if torch.onnx.is_in_onnx_export() and self.bbox_norm_type == 1:
        ...
        if self.bbox_norm_type == 1:
            final_box_preds = denormalize_bbox_type1(bbox_preds, self.pc_range)
        else:
            final_box_preds = denormalize_bbox_type2(bbox_preds, self.pc_range)
        final_scores = scores
        final_preds = labels
    ...
    def decode(self, preds_dicts):
        ...
        Returns:
            list[dict]: Decoded boxes.
        """
        # Not fully verified for PETR, Far3D, StreamPETR
