...
def get_label_anno_custom(label_path):
    annotations = {}
    annotations.update({
        'name': [],
        'truncated': [],
    ...
    content = [line.strip().split(' ') for line in lines]
    annotations['name'] = np.array([class_to_name[(int)(x[0])] for x in content])
    annotations['truncated'] = np.array([0 for x in content])
    annotations['occluded'] = np.array([0 for x in content])
    annotations['alpha'] = np.array([0 for x in content])
    annotations['bbox'] = np.array(
        [[0.0 for info in x[4:8]] for x in content]).reshape(-1, 4)
    # dimensions will convert hwl format to standard lhw(camera) format.
    annotations['dimensions'] = np.array(
        [[float(info) for info in x[5:8]] for x in content])
    annotations['location'] = np.array(
        [[float(info) for info in x[2:5]] for x in content])
    annotations['rotation_y'] = np.array(
        [float(x[8]) for x in content]).reshape(-1)

    annotations['score'] = np.array([float(x[1]) for x in content])

    return annotations

def get_label_annos(label_folder, image_ids=None, en_tidl_custom_format=False):
