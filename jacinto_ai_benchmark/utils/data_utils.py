...
def download_url(url, root, filename=None, md5=None):
    ...
    if check_integrity(fpath, md5):
        ...
        with progiter.ProgIter(total=None) as pbar:
            def gen_bar_updater():
                def bar_update(count, block_size, total_size):
                    ...
                    if progress_bytes>0:
                        pbar.update(progress_bytes - pbar._now_idx)
                    #
                #
                return bar_update
...
def _save_response_content(response, destination, chunk_size=32768):
    with open(destination, "wb") as f:
        pbar = progiter.ProgIter(total=None)
        progress = 0
...
def _is_archive(from_path):
    return _is_tar(from_path) or _is_targz(from_path) or _is_gzip(from_path) or _is_zip(from_path)
