...
class Far3DPandaSetDataset(PandaSetDataset):
    r"""Pandaset Dataset.
    ...
    def _set_sequence_group_flag(self):
        ...
        for idx in range(len(self.data_list)):
            ...
            curr_scene_token =  self.data_list[idx].get('scene_token') 
            if prev_scene_token is None:
                prev_scene_token = curr_scene_token
            if curr_scene_token != prev_scene_token:
                # Not first frame and new scene_token -> new sequence
                prev_scene_token = curr_scene_token
                curr_sequence += 1
                res[-1] = curr_sequence
            else:
    ...
    def get_data_info(self, index):
        ...
        new_labels2d = []
        for each in labels2d:
            new_labels = []
            for label in each:
                new_labels.append(self.label_mapping[label])
            new_labels2d.append(np.array(new_labels))
        info['labels2d'] =(new_labels2d)
