...
from mmengine.registry import MODELS, TASK_UTILS, DATA_SAMPLERS
...
class Sparse4DHead(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.instance_bank = build(instance_bank, MODELS)
        self.anchor_encoder = build(anchor_encoder, MODELS)
        self.depth_module = build(depth_module, MODELS)
        self.dense_depth_module = build(dense_depth_module, MODELS)
        self.kps_generator = build(kps_generator, MODELS)
        self.sampler = build(sampler, DATA_SAMPLERS)
        self.decoder = build(decoder, TASK_UTILS)
        self.loss_cls = build(loss_cls, MODELS)
        self.loss_reg = build(loss_reg, MODELS)
        self.op_config_map = {
