...
class VectorEvaluate(object):
    ...
    def __init__(self, dataset_cfg: Config) -> None:
        self.dataloader = Runner.build_dataloader(dataset_cfg)
        classes = dataset_cfg.dataset.map_classes
        self.cat2id = {cls: i for i, cls in enumerate(classes)}
        self.id2cat = {v: k for k, v in self.cat2id.items()}
        self.n_workers = dataset_cfg.num_workers
    ...
    def gts(self) -> Dict[str, Dict[int, List[NDArray]]]:
        ...
        gts = {}
        pbar = mmengine.utils.ProgressBar(len(self.dataloader))
        for data in self.dataloader:
            token = deepcopy(data['data_samples'][0].metainfo['token'])
            gt = deepcopy(data['data_samples'][0].vectors)
            gts[token] = gt
            pbar.update()
    ...
    def evaluate(self, 
        ...
        '''
        results = mmengine.load(result_path)
        ...
        sum_mAP = 0
        pbar = mmengine.utils.ProgressBar(len(self.id2cat))
        for label in self.id2cat.keys():
