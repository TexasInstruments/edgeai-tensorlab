...
from projects_edgeai.SparseDrive.sparsedrive.datasets.utils import box3d_to_corners
...
def planning_eval(results, eval_config, logger):
    ...
    dataloader = Runner.build_dataloader(eval_config)
    planning_metrics = PlanningMetric()
    for i, data in enumerate(tqdm(dataloader)):
       # Temporary break
        if i == len(results):
            break
        sdc_planning = data['data_samples'][0].gt_ego_fut_trajs.cumsum(dim=-2).unsqueeze(0)
        sdc_planning_mask = data['data_samples'][0].gt_ego_fut_masks.unsqueeze(-1).repeat(1, 1, 2).unsqueeze(1)
        command = data['data_samples'][0].gt_ego_fut_cmd.argmax(dim=-1).item()
        fut_boxes = data['data_samples'][0].fut_boxes
        if not sdc_planning_mask.all(): ## for incomplete gt, we do not count this sample
            continue
        res = results[i]
        pred_sdc_traj = res['pred_instances_3d']['final_planning'].unsqueeze(0)
