...
from ..models.attention import gen_sineembed_for_position
...
class MotionPlanningHead(BaseModule):
    def __init__(
        ...
        self.instance_queue = MODELS.build(instance_queue)
        self.motion_sampler = TASK_UTILS.build(motion_sampler)
        self.planning_sampler = TASK_UTILS.build(planning_sampler)
        self.motion_decoder = TASK_UTILS.build(motion_decoder)
        self.planning_decoder = TASK_UTILS.build(planning_decoder)
        self.op_config_map = {
            "temp_gnn": [temp_graph_model, MODELS],
            "gnn": [graph_model, MODELS],
            "cross_gnn": [cross_graph_model, MODELS],
        ...
        self.motion_loss_cls = MODELS.build(motion_loss_cls)
        self.motion_loss_reg = MODELS.build(motion_loss_reg)
        self.plan_loss_cls = MODELS.build(plan_loss_cls)
        self.plan_loss_reg = MODELS.build(plan_loss_reg)
        self.plan_loss_status = MODELS.build(plan_loss_status)
