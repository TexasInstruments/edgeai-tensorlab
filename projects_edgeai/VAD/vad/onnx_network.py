...
def precompute_bev_info(model):
    """
    Pre-compute the voxel info 
    """
    bev_query = model.bev_embedding.weight.to(torch.float32)
    bev_query = bev_query.unsqueeze(1)

    ref_3d = get_reference_points(
        model.bev_h, model.bev_w, model.pc_range[5]-model.pc_range[2], model.num_points_in_pillar,
    ...
    reference_points_cam, bev_mask = point_sampling(
        ref_3d, model.pc_range, model.img_metas)
    ...
    for mask_per_img in bev_mask:
        ...
        index_query_per_img = nzindex.new_ones(model.bev_h * model.bev_w)*model.bev_h * model.bev_w
    ...
    can_bus = bev_query.new_tensor([each['can_bus'] for each in model.img_metas])

    delta_x = np.array([each['can_bus'][0] for each in model.img_metas])
    delta_y = np.array([each['can_bus'][1] for each in model.img_metas])
    ego_angle = np.array(
        [each['can_bus'][-2] / np.pi * 180 for each in model.img_metas])
    grid_length_y = model.real_h / model.bev_h
    grid_length_x = model.real_w / model.bev_w
    translation_length = np.sqrt(delta_x ** 2 + delta_y ** 2)
    translation_angle = np.arctan2(delta_y, delta_x) / np.pi * 180
    bev_angle = ego_angle - translation_angle
    shift_y = translation_length * \
        np.cos(bev_angle / 180 * np.pi) / grid_length_y / model.bev_h
    shift_x = translation_length * \
        np.sin(bev_angle / 180 * np.pi) / grid_length_x / model.bev_w
...
def get_ego_features(model):
    ego_his_trajs_t = []
    ego_fut_cmd_t   = []
    ego_lcf_feat_t  = []
    for _, img_metas in enumerate(model.img_metas):
    ...
    return ego_his_trajs, ego_fut_cmd, ego_lcf_feat

def compute_rotation_matrix(model):
    height = model.bev_h
    width  = model.bev_w
    oh = height
    ow = width
    ...
    center_f = [0.0, 0.0]
    center_f = [1.0 * (c - s * 0.5) for c, s in zip(model.rotate_center, [width, height])]

    angle = model.img_metas[0]['can_bus'][-1]
...
class VAD_export_model(nn.Module):
    ...
    def set_img_metas(self, img_metas):
    ...
    def forward(self,
        ...
        bbox_list = self.pts_bbox_head.get_bboxes(outs, self.img_metas, rescale=True)
...
class VAD_export_img_backbone(nn.Module):
    def __init__(self,
                 img_backbone,
                 img_neck,
                 ):
...
class VAD_export_pts_bbox_head(nn.Module):
    def __init__(self,
                 pts_bbox_head,
                 video_test_mode,
                 map_pred2result
    ...
    def forward(self,
        ...
        outs = self.pts_bbox_head(img_feats, self.img_metas, prev_bev=prev_bev,
                                  rotation_grid=rotation_grid,
                                  reference_points_cam=reference_points_cam,
                                  bev_mask_count=bev_mask_count,
                                  bev_valid_indices=bev_valid_indices,
