...
def get_args_parser(add_help = True):
    ...
    parser.add_argument('--original-accuracy','--acc',dest='orig_acc', default=78, type=int, help='accuracy of original model')
...
def main(args):
    ...
    train_args = ['--data-path',args.data_path,'--gpus',args.gpus,'--parallel','1','--batch-size',args.batch_size,'--model',args.model,'--opset-version',args.opset_version,'--val-resize-size',args.val_resize_size,'--val-crop-size',args.val_crop_size, '--output-dir',args.output_dir,"--print-freq",'200','--weights',args.weights,'--test-only','--model-surgery','2']
    ...
    orig_model:str = MODEL_SURGERY_NAMES_LITE2ORIGINAL[args.model][0]
    ...
    if not os.path.exists(args.result_path):
        ...
        df = pd.DataFrame({col:list() for col in columns})
    else:  
