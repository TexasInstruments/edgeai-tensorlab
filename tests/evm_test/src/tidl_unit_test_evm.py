...
class TIDLUnitTestEVM():
    def __init__(self, evm_config, edgeai_benchmark_path, ip_address, dataset_dir_path, model_artifacts_path=None, reboot_type="soft", evm_local_ip="", logs_dir=None):
        ...
        if self.logs_dir == None:
            self.logs_dir = f"evm_test_logs/TIDL_UNIT_TEST/{self.soc}"
    ...
    def init_setup(self):
        ...
        if status:
            command = f"cd && EAI_BENCHMARK_MOUNT_PATH={self.eai_benchmark_mount_path} TEST_SUITE=TIDL_UNIT_TEST LOCAL_IP={self.evm_local_ip} TIDL_UNIT_TEST_DATASET_MOUNT_PATH={self.dataset_dir_mount_path} TIDL_UNIT_TEST_MODEL_ARTIFACTS_MOUNT_PATH={self.model_artifacts_mount_path} ./setup_eai_benchmark.sh"
