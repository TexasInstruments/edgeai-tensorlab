...
By default, all operator tests are distributed across multiple PCs, which is
beneficial for running time-consuming operators. However, the script includes
an option to restrict an operator to a single PC.
`restrict_to_single_pc` is an argument to the python file which ensures that
tests for specific operators are not distributed across multiple PCs.
...
```bash
# This will run defined operators tests across PCs defined by distributed_test_config.json
# in chunks i.e. all the tests will get distributed across PCs.
# Tests under operators defined in restrict_to_single_pc will not be distributed across PCs and will run on single PC.

python3 distributed_test.py  --operators="Abs ArgMax Cos Sin" --restrict_to_single_pc="ArgMax Sin" --SOC=AM68A --tidl_tools_path=*link to tidl_tools tarball* --compile_without_nc=1 --compile_with_nc=1 --run_ref=1 --run_natc=1 --run_ci=1  --tidl_offload=1
```
