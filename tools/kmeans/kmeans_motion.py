...
for idx in tqdm(range(len(data_infos))):
    ...
    for instance in data_infos[idx]['instances']:
        ...
        velos.append(np.array(instance['velocity']))

    boxes = np.array(boxes)
    labels = np.array(labels)
    velos = np.array(velos)
    fut_masks = data_infos[idx]['gt_agent_fut_masks']
    trajs = data_infos[idx]['gt_agent_fut_trajs']
...
for i, _ in enumerate(CLASSES):
    if len(intention[i]) == 0:
        # It shouldn't happen though for large enough data
        continue
...
np.save(f'data/nuscenes/kmeans/kmeans_motion_{K}.npy', clusters)